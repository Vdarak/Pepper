...
export default function FlagsPennantsPage() {
  ...
  useEffect(() => {
    async function fetchResults() {
      try {
        const response = await fetch('/api/scanners/flags-pennants');
        if (!response.ok) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 p-8 bg-gray-50">
        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-2">Flags & Pennants Scanner</h1>
          <p className="text-gray-600">
            Continuation patterns with strong directional move followed by consolidation
          </p>
        </div>
