...
export default function VCPPage() {
  ...
  useEffect(() => {
    async function fetchResults() {
      try {
        const response = await fetch('/api/scanners/vcp');
        if (!response.ok) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 p-8 bg-gray-50">
        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-2">VCP Scanner</h1>
          <p className="text-gray-600">
            Volatility Contraction Pattern - stocks with tightening bases
          </p>
        </div>
